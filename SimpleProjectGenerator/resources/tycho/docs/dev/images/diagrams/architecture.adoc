...
= Siemens EDA Software {n} VSE Analysis : Architecture
// Author line
...
`(~)` is a shortcut for `com.siemens.eda.vseanalysis` prefix.
...
| `(~).xmlexport`        | JAXB classes from XSD and descriptors of dynamic Properties.
| `(~).design`           | Definition of Representations and diagram extensions for 
                           Capella.
| `(~).rules.validation` | Rules to verify model validity.
| `(~).runtime`          | XML transformation and Process monitoring.
...
....
include::{includesdir}/diagrams/plugin_dependencies.plantuml[]
....
...
A Oomph setup is available at _/com.siemens.sw.eda.vseanalysis.parent/oomph_.
...
See _/com.siemens.sw.eda.vseanalysis.parent/preferences_ folder for detail indications.

==== Internationalization (I18n) in Design

Using id mechanism, i18n labels are automatically set in ODesign. +
_com.siemens.eda.vseanalysis.design/plugin.properties_ must be updated accordingly
